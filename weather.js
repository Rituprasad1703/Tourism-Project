...
function getDateTime() { //to update date and time
	let now=new Date(),
	hour=now.getHours(),
	...
	hour=hour%12;
	//hour=hour?hour:12;  //the zero should be 12 bcuz 12%12=0 there's no 0
	hour=hour<10?"0"+hour:hour; //add prefix 0 if <10
	minute=minute<10?"0"+minute:minute; //12 hour format
	let dayString=days[now.getDay()];
	return `${dayString}, ${hour}:${minute}`;
}
date.innerText=getDateTime();

setInterval(()=>{   //update time every second
	date.innerText=getDateTime();
	...
	fetch("https://geolocation-db.com/json/",
		{
			method:"GET",
			headers:{},
		})   //bcuz of a semi-colon I gave here before, 
	       	 //time wasn't coming haha
...
function getWeatherData(city,unit,hourlyOrWeek) {
	const APIkey="VXYN6GT5H5YFXLFF7L7Y4MLQM";
	fetch(
		`https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/${city}?unitGroup=metric&key=VXYN6GT5H5YFXLFF7L7Y4MLQM&contentType=json`,
		{
			method:"GET",
			headers:{},
		}
...
function updateForecast(data,unit,type) {
	...
	for(let i=0;i<numCards;i++)
	{
		...
		card.classList.add("card"); //one card is created again 
									//using HTML logic only
		...
		let iconCondition=data[day].icon; //fetching the icon of the day
...
function convertTimeTo12HourFormat(time) {
	let hour=time.split(":")[0]; //first element i.e., LHS of time
								 //string (15:00) is in hour
	let minute=time.split(":")[1];	//2nd element is in minute
...
search.addEventListener("input",function (e){
	...
	i,
	val=this.value;
	if(!val){   //if there's nothing search input do nothing
		return false;
	}
	currentFocus=-1;
	...
	{
		//check if items start with same letter which are in input
		...
		if(cities[i].substr(0,val.length).toUpperCase()==val.toUpperCase())
		{
			...
			//adding eventListener or suggestion
			b.addEventListener("click",function(e){
				//on click set the search input val with clicked suggestion
				//value
...
search.addEventListener("keydown",function(e)
{
	...
	if(x) x=x.getElementsByTagName("li");
		if(e.keyCode==40){
			...
			currentFocus++;
			//lets create a function to add a active suggestion
			addActive(x);
		}
	...
	if(e.keyCode==13)
	{
		//if enter is pressed add the current selected removeSuggestion
			e.preventDefault();
		if(currentFocus>-1){
...
function addActive(x){
	//if there's no suggestion return as it is
	if(!x) return false;
	removeActive(x);
...
//just to remove the suggestion for the next suggestion to come
function removeSuggestion(){
	//select the ul which is being added on search input
	var x=document.getElementById("suggestions");
